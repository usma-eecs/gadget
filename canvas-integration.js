...
// canvas assigns unique id to files every time they're changed
// so we have to hunt down the current id for gadget/editor.html
// do it lazily so we don't request it on pages without gadgets
const iframe = callback => {
iframe.request = iframe.request || 
  $.get('/api/v1/courses/' + course + '/files?search_term=editor.html');

iframe.request.then(results => {
  if (results.length == 0) {
    console.log("master: gadget support files haven't been uploaded to this course!");
  } else {
    var element = $('<iframe>', {
      height: 350,
      width: '100%',
      ...
      src: '/courses/' + course + '/files/' + results[0].id + '/file_preview?annotate=0'
    });
    callback(element);
  }
});
