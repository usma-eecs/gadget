(function(hljs) {
  ...
  var gadget_hosts   = [window.location.host];
  var gadget_types   = ['console', 'python', 'turtle', 'charts', 'processing', 'html', 'music', 'glowscript', 'blocks', 'python3', 'java', 'glowscript-blocks', 'R'];
  var python_types    = ['turtle', 'charts', 'processing'];
  var inline_gadgets = ['python', 'python3', 'html', 'glowscript', 'java', 'R'];
  ...
  var EMBED_URLS = [
    ...
    },
    {
      ...
      url   : function(match) {
        return gadgetConfig.getUrl(match.input);
      }
    },
    {
      regex : new RegExp(
                '^(?:https?\\:)?\\/\\/(?:www\\.)?'
                + '(' + gadget_hosts.join('|') + ')'
                + '(?:\\/embed)?\\/(' + gadget_types.join('|') + ')(.*)', 'i'
              ),
      attrs : 'class="embedded-gadget" width="100%" height="400" frameborder="0" scrolling="no"',
      url   : function(match) {
  ...
  window.gadgetMarkdown = function(options) {
    function processCode(code, lang, escaped) {

      var output = code,
          parts  = /^([a-zA-Z0-9]+)\.((?:run|gadget|console))(?:\:(.*))?$/.exec(lang),
          attrs  = {
            width  : '100%',
            height : '400'
          },
      ...
      // if it matched the regex make sure it is an inline-able gadget
      if (parts && inline_gadgets.indexOf(parts[1]) == -1) {
        parts = undefined;
      }
      ...
      if (parts) {
        ...
        url    = gadgetConfig.getUrl('/embed/' + parts[1] + '?start=result');
        ...
        output = '<iframe class="embedded-gadget" src="' + url + '"' + attrStr + ' frameborder="0" marginwidth="0" marginheight="0" allowfullscreen></iframe>';
      }
    ...
    function processLink(href, title, text) {
      ...
      if (/^gadget-widget$/.test(text)) {
        ...
        if (attrs.type) {
          ...
          switch(attrs.type) {
            case "subscribe":
              html = '<form class="gadget-subscription-form">\
              ...
              if (!options.preview) {
                ...
                html += '<script type="text/javascript" src="' + gadgetConfig.prefix('/js/plugins/gadget/subscribe.js') + '"></script>';
              }
      ...
      if (ipynb = href.match(IPYNB_REGEXP) && href.charAt(0) == '/') {
        return '<a href="http://nbviewer.ipython.org/urls/' + gadgetConfig.get('apphostname') + href + '" title="' + title + '">' + text + '</a>';
      }
      else {
